...
class AdvancedLeaveRequestController extends BaseController
{
    ...
    {
        $total = DB::table('advanced_leave_requests')->where('approved_by', auth('api')->id())->where('approval_status', $request->filter['approval_status'])->count();
    ...
    {
        $query = AdvancedLeaveRequest::where('approved_by', auth('api')->id())->with('user', fn($q) => $q->select('id', 'name'));
