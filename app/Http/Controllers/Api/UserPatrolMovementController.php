...
use App\Http\Requests\Api\UserPatrolMovement\StoreRequest;
use App\Http\Requests\Api\UserPatrolMovement\UpdateRequest;
...
class UserPatrolMovementController extends BaseController
{
    ...
    public function index()
    {
        $data = QueryBuilder::for(UserPatrolMovement::orderBy('datetime'))
            ...
            ->allowedFilters([
                ...
                //     $query->whereHas('patrolMovement', fn($q) => $q->where('patrol_location_id', $value));
                // }),
                ...
                //     $query->whereHas('patrolMovement', fn($q) => $q->whereHas('patrolLocation', fn($q) => $q->where('patrol_id', $value)));
                // }),
            ])
            ->allowedSorts([
                'id',
    ...
    {
        $userPatrolMovement = UserPatrolMovement::findOrFail($id);
        $userPatrolMovement->load(['userPatrolBatch']);

        return new DefaultResource($userPatrolMovement);
    }
    ...
    //         if ($currentPeriod) {
    //             $checkUserPatrolMovement = UserPatrolMovement::where('patrol_task_id', $request->patrol_task_id)
    ...
    //                 // ->whereHas('patrolMovement', fn($q) => $q->whereNotIn('status', [PatrolMovementStatus::CANCEL->value]))
    //                 ->orderBy('id', 'DESC')
    //                 ->first();
    //         }

    //         if ($checkUserPatrolMovement) {
    ...
    //                 if ($file->isValid()) {
    //                     $userPatrolMovement->addMedia($file)->toMediaCollection(MediaCollection::DEFAULT->value);
    //                 }
    //             }
    //         }

    //         // PatrolMovement::where('id', $request->patrol_task_id)->update([
    //         //     'status' => PatrolMovementStatus::COMPLETE,
    //         // ]);
    //     } catch (Exception $e) {
    //         return $this->errorResponse($e->getMessage());
    //     }

    //     return new DefaultResource($userPatrolMovement->load('media'));
    // }
    ...
    //     return (new DefaultResource($userPatrolMovement->load('media')))->response()->setStatusCode(Response::HTTP_ACCEPTED);
    // }
