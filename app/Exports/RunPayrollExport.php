...
class RunPayrollExport implements FromView
{
    ...
    {
        $payrollComponents = PayrollComponent::where('company_id', $this->runPayroll->company_id)->get(['id', 'name', 'type']);

        return view('api.exports.payroll.run-payroll', [
            'runPayroll' => $this->runPayroll,
            'payrollComponentType' => PayrollComponentType::class,
            'allowances' => $payrollComponents->where('type', PayrollComponentType::ALLOWANCE),
